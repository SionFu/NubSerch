...
/* 
 ...
 http://api.map.baidu.com/place/v2/search?query=饭店&region=北京&output=json&ak=clZ8KNbp57z41BRCVjgvxyK98ew111Ao
 */
...
-(void)getDataWithBaiDuLocalWord:(NSString *)localWord andKeyWord:(NSString *)keyWord andPage:(NSInteger)page {
    NSString *url = [NSString stringWithFormat:@"%@query=%@&region=%@&output=json&page_num=%ld&ak=%@",BiaduApi,keyWord,localWord, (long)page, BiaduKEY];
    NSString * encodedString = (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes( kCFAllocatorDefault, (CFStringRef)url, NULL, NULL,  kCFStringEncodingUTF8 ));
    AFHTTPRequestOperationManager *manger = [AFHTTPRequestOperationManager manager];
    [manger GET:encodedString parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        [HSDataManger sharedHSDataManger].getDicData = responseObject;
        [self.getDataDelegate getBaiDuDataSuccess];
        NSLog(@"%@",url);
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
        [self.getDataDelegate getDataFaild];
    }];
